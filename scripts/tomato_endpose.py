...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QPen, QColor, QFont, QPolygonF, QBrush
    ...
    if len(contours) > 0:
        ...
        try:
            ...
            if defects is not None:
                ...
                if cutting_points:
                    return min(cutting_points, key=lambda p: np.sqrt((p[0]-stem_center[0])**2 + (p[1]-stem_center[1])**2))
        except cv2.error:
            pass
    return stem_center
...
class ImageDisplayWidget(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if len(self._screen_points) > 0:
            ...
            for p in self._screen_points: painter.drawPoint(p)
    ...
    def _screen_to_img(self, pt):
        img_x = np.clip((pt.x() - self._pixmap_offset[0]) / self._scale_x, 0, self._pixmap.width() - 1)
        img_y = np.clip((pt.y() - self._pixmap_offset[1]) / self._scale_y, 0, self._pixmap.height() - 1)
        return (img_x, img_y)

    def _img_to_screen(self, x, y):
        if not hasattr(self, '_scale_x'): return QPoint()
        return QPoint(int(x * self._scale_x + self._pixmap_offset[0]), int(y * self._scale_y + self._pixmap_offset[1]))

    def mousePressEvent(self, event):
        if self._is_polygon_closed: self.clear_selection()
        if event.button() == Qt.LeftButton:
            self._screen_points.append(event.pos())
            self._cutting_point_screen = None
            self.update()
            if len(self._screen_points) >= 3: self._update_mask_and_calculate()
    ...
    def _update_mask_and_calculate(self):
        if len(self._screen_points) < 3 or not hasattr(self, '_latest_cv_image'): return
        ...
        if stem_center is None: return
        cutting_point = find_cutting_point(mask, stem_center, self._latest_cv_image)
        if cutting_point is not None:
            self._cutting_point_screen = self._img_to_screen(cutting_point[0], cutting_point[1])
            self.update()
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"✅ 相机启动成功 (深度分辨率: {depth_profile.get_width()}x{depth_profile.get_height()}, 视差: 128)")
            ...
            while self._running:
                ...
                self.data_ready.emit(depth_data, depth_frame.get_depth_scale(), (color_frame.get_width(), color_frame.get_height()))

                # 读取 LRM 数据
                if self.device:
                    try:
                        lrm_dist_mm = self.device.get_int_property(OBPropertyID.OB_PROP_LDP_MEASURE_DISTANCE_INT)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.depth_intrinsics = np.array([[612.287415, 0, 638.468506],
                                           [0, 612.246033, 359.933319],
    ...
    def init_ros(self):
        ...
        try:
            ...
            if not rospy.is_shutdown():
                ...
                rospy.init_node('tomato_picking_gui', anonymous=True, disable_signals=True)
                
                # 等待服务可用（超时5秒）
                rospy.wait_for_service('/piper_server/eef_cmd', timeout=5.0)
                
                # 创建服务代理
                self.cmd_service_client = rospy.ServiceProxy('/piper_server/eef_cmd', piper_cmd)
    ...
    def init_ui(self):
        ...
        self.btn_reset.setStyleSheet("background-color: #3498db; color: white; font-size:14px; font-weight:bold;")
        ...
        self.btn_send_cmd.setStyleSheet("background-color: #27ae60; color: white; font-size:14px; font-weight:bold;")
    ...
    def on_roi_selected(self, cutting_pixel, mask):
        ...
        depth_mask = cv2.resize(mask, (depth_w, depth_h), interpolation=cv2.INTER_NEAREST)
        depth_raw = self.get_valid_depth_around(self.current_depth_data, du, dv, depth_mask)
